...
# Calcular tiempo promedio a los 5 espacios culturales más cercanos
origenes <- origenes %>%
  mutate(
...
# =======================================
# 8) Puntaje 0/1 según tiempo umbral de 9 min
# =======================================
origenes <- origenes %>%
  mutate(
    acceso_PcD = if_else(time_PcD_min <= 9, 1L, 0L),
    acceso_PsD = if_else(time_PsD_min <= 9, 1L, 0L)
  )
...
st_write(manzanas_final, "puntaje_distancia_espacios_culturales.geojson", delete_layer = TRUE)
