...
# =======================================
# 3) Calcular longitudes y tiempos (PcD / PsD)
# =======================================
net <- net %>%
  activate("edges") %>%
  mutate(
...
# Eliminar destinos duplicados en el mismo nodo
destinos <- destinos %>%
  distinct(nearest_node, .keep_all = TRUE)

# =======================================
# 6) Grafo igraph con pesos
# =======================================
grafo <- net %>% as.igraph()
...
# =======================================
# 7) Distancia promedio a los 5 destinos más cercanos
# =======================================
...
# =======================================
# 8) Puntaje 0/1 según tiempo umbral — actualizado a 29 min
# =======================================
origenes <- origenes %>%
  mutate(
    acceso_PcD = if_else(time_PcD_min <= 29, 1L, 0L),
    acceso_PsD = if_else(time_PsD_min <= 29, 1L, 0L)
  )

# Agrupar por manzana (CFRM)
...
# Guardar
st_write(manzanas_final, "puntaje_distancia_equip_asistencial.geojson", delete_layer = TRUE)
