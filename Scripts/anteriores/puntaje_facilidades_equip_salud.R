...
destinos_puntaje <- destinos %>%
  mutate(
    puntaje_base = case_when(
      Tipo == "Centro de Salud - Nivel 1" ~ 0.25,
      Tipo == "Centro médico barrial" ~ 0.25,
      Tipo == "Banco de Elementos Ortopédicos" ~ 0.25,
      Tipo == "Centro Oncológico Integral" ~ 0.75,
      Tipo == "Hospital de Niños" ~ 0.75,
      Tipo == "Hospital Especializado" ~ 0.75,
      Tipo == "Hospital General de Agudos" ~ 0.50,
      TRUE ~ 0  # Por defecto si no coincide
    ),
    dest_idx = row_number()  # ID único para st_nn
  )

manzanas <- st_read("puntaje_distancia_equip_salud.geojson")
# ==========================================
...
#    Para 23 min → 2.5 km/h = ~958 m | 5 km/h = ~1917 m
# ==========================================
vecinos_PcD <- st_nn(manzanas_PcD, destinos_puntaje, k = 5, maxdist = 958)
vecinos_PsD <- st_nn(manzanas_PsD, destinos_puntaje, k = 5, maxdist = 1917)
...
# ==========================================
st_write(manzanas_final, "manzanas_final_puntaje_salud.geojson", delete_dsn = TRUE)
