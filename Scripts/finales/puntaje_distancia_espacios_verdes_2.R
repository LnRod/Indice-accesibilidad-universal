...
# ===========================
# 2) Calcular longitudes y pesos para cada perfil (network)
# ===========================
net <- net %>%
  activate("edges") %>%
  mutate(
...
# =======================================
# 3) Pesos: longitudes y tiempos PcD / PsD
# =======================================
net <- net %>%
  activate("edges") %>%
  mutate(
...
# =======================================
# 6) Distancias reales a destinos más cercanos
# =======================================
...
calc_min_time <- function(from, destino_nodes, weights, N = 5) {
  ...
  # Selecciona los N tiempos más cortos
  dist_sorted <- sort(dist)
  mean5 <- mean(dist_sorted[1:min(N, length(dist_sorted))])
  return(mean5)
}

# Calcular tiempo promedio a los 5 espacios más cercanos
origenes <- origenes %>%
  mutate(
...
# =======================================
# 7) Puntaje 0/1 según tiempo umbral
# =======================================
origenes <- origenes %>%
  mutate(
    acceso_PcD = if_else(time_PcD_min <= 15, 1L, 0L), #Cambiar segun tiempo umbral
    acceso_PsD = if_else(time_PsD_min <= 15, 1L, 0L)
  )
...
manzanas <- st_read("manzanas_geom.geojson") %>% st_make_valid()

manzanas_final <- manzanas %>%
  left_join(origenes_unique, by = "CFRM")

manzanas_final <- manzanas_final %>% select(-ID, -perimetro)
...
st_write(manzanas_final, "puntaje_distancia_espacios_verdes_MZ.geojson", delete_layer = TRUE)


#####Puntaje por facilidades (acumulativo)
