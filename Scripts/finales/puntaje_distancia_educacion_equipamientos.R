...
# *** Se actualiza el destino a Equipamientos Educativos ***
...
# =======================================
## FUNCIÓN ACTUALIZADA con la LÓGICA DE TIEMPO para "Equipamientos de EDUCACIÓN"
# Promedio de tiempo de acceso a Equipamientos educativos adaptados a PcD = 8 minutos.
# Si < 3 min = 1. Si [3, 8) min = 0.5. Si [8, 12) min = 0.25. Si >= 12 min = 0.
calcular_puntaje_gradual <- function(tiempo_min) {
  case_when(
    tiempo_min < 3 ~ 1.00,        # Menos de 3 minutos
    tiempo_min < 8 ~ 0.50,        # Entre 3 y menos de 8 minutos
    tiempo_min < 12 ~ 0.25,       # Entre 8 y menos de 12 minutos
    TRUE ~ 0.00                   # 12 minutos o más
  )
}
origenes <- origenes %>%
  mutate(
...
manzanas_final <- manzanas %>%
  ...
  # Actualizar nombres de columnas para EDUCACIÓN
  rename(
    puntaje_dist_educacion_PcD = puntaje_PcD,
    puntaje_dist_educacion_PsD = puntaje_PsD
  )
...
# =======================================
## FUNCIÓN ACTUALIZADA con las categorías de tiempo para EDUCACIÓN
crear_tabla_resumen <- function(datos_sf, perfil) {
  
  col_puntaje <- paste0("puntaje_dist_educacion_", perfil) # Usar el nuevo nombre de columna
  # Velocidad en metros por minuto (m/min)
  velocidad_mpm <- if (perfil == "PcD") (2.5 * 1000) / 60 else (5 * 1000) / 60
  
  # Categorías de tiempo ACTUALIZADAS para Equipamientos de EDUCACIÓN
  categorias_tiempo <- c(
    "1 - Menos de 3 min", 
    "0,5 - Entre 3 y 8 min", 
    "0,25 - Entre 8 y 12 min", 
    "0 - Mas de 12 min"
  )
  umbrales_min <- c(3, 8, 12, NA) # Límite superior para la distancia/tiempo
  ...
  resumen <- tibble(
    ...
    `Distancia Aprox (m)` = c(
      round(3 * velocidad_mpm), # Max 3 min
      round(8 * velocidad_mpm), # Max 8 min
      round(12 * velocidad_mpm), # Max 12 min
      NA
    )
  ) %>%
...
write.csv(tabla_combinada, "C:/Users/pcusu/Desktop/puntaje_distancia_EDUCACION_tabla.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
crear_mapa_accesibilidad <- function(datos_sf, perfil, tabla_resumen) {
  ...
  col_puntaje <- paste0("puntaje_dist_educacion_", perfil) # Nuevo nombre de columna
  # Título actualizado para Equipamientos de Educación
  titulo_mapa <- paste("Acceso a Equipamientos de Educación - Perfil", perfil)
...
st_write(manzanas_final, "C:/Users/pcusu/Desktop/puntaje_distancia_educacion_equipamientos.geojson", delete_layer = TRUE)