...
# *** Se actualiza el destino a Equipamientos Sociales y Asistenciales ***
destinos <- st_read("C:/Users/pcusu/Desktop/Accesibilidad_CABA/Datos/Datos_indice/equip_asis.geojson") %>% st_make_valid()
...
# FILTRAR ORÍGENES que no se pudieron snap (poco común para centroides)
origenes <- origenes %>% 
  filter(!is.na(nearest_node))

destinos$nearest_node <- snap_points(destinos, nodos)
# FILTRAR DESTINOS que no se pudieron snap (importante si hay geometrías vacías/nulas)
destinos <- destinos %>%
  filter(!is.na(nearest_node)) %>% 
...
# =======================================
## FUNCIÓN ACTUALIZADA con la LÓGICA DE TIEMPO para "Equipamientos Sociales y Asistenciales"
# Promedio de tiempo de acceso = 29 minutos.
# Si < 24 min = 1. Si [24, 29) min = 0.5. Si [29, 34) min = 0.25. Si >= 34 min = 0.
calcular_puntaje_gradual <- function(tiempo_min) {
  case_when(
    tiempo_min < 24 ~ 1.00,        # Menos de 24 minutos
    tiempo_min < 29 ~ 0.50,        # Entre 24 y menos de 29 minutos
    tiempo_min < 34 ~ 0.25,        # Entre 29 y menos de 34 minutos
    TRUE ~ 0.00                    # 34 minutos o más
  )
}
origenes <- origenes %>%
  mutate(
...
manzanas_final <- manzanas %>%
  ...
  # Actualizar nombres de columnas para ASISTENCIAL
  rename(
    puntaje_dist_asistencial_PcD = puntaje_PcD,
    puntaje_dist_asistencial_PsD = puntaje_PsD
  )
...
# =======================================
## FUNCIÓN ACTUALIZADA con las categorías de tiempo para ASISTENCIAL
crear_tabla_resumen <- function(datos_sf, perfil) {
  
  col_puntaje <- paste0("puntaje_dist_asistencial_", perfil) # Usar el nuevo nombre de columna
  # Velocidad en metros por minuto (m/min)
  velocidad_mpm <- if (perfil == "PcD") (2.5 * 1000) / 60 else (5 * 1000) / 60
  
  # Categorías de tiempo ACTUALIZADAS para Equipamientos ASISTENCIALES
  categorias_tiempo <- c(
    "1 - Menos de 24 min", 
    "0,5 - Entre 24 y 29 min", 
    "0,25 - Entre 29 y 34 min", 
    "0 - Mas de 34 min"
  )
  umbrales_min <- c(24, 29, 34, NA) # Límite superior para la distancia/tiempo
  ...
  resumen <- tibble(
    ...
    `Distancia Aprox (m)` = c(
      round(24 * velocidad_mpm), # Max 24 min
      round(29 * velocidad_mpm), # Max 29 min
      round(34 * velocidad_mpm), # Max 34 min
      NA
    )
  ) %>%
...
write.csv(tabla_combinada, "C:/Users/pcusu/Desktop/puntaje_distancia_ASISTENCIAL_tabla.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
crear_mapa_accesibilidad <- function(datos_sf, perfil, tabla_resumen) {
  ...
  col_puntaje <- paste0("puntaje_dist_asistencial_", perfil) # Nuevo nombre de columna
  # Título actualizado para Equipamientos ASISTENCIALES
  titulo_mapa <- paste("Acceso a Equipamientos Sociales y Asistenciales - Perfil", perfil)
...
st_write(manzanas_final, "C:/Users/pcusu/Desktop/puntaje_distancia_asistencial_equipamientos.geojson", delete_layer = TRUE)
