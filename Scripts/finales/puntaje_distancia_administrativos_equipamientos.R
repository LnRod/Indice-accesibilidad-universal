...
# *** Se actualiza el destino a Equipamientos Administrativos ***
...
# =======================================
## FUNCIÓN ACTUALIZADA con la LÓGICA DE TIEMPO para "Equipamientos Administrativos"
# Promedio de tiempo de acceso = 26 minutos.
# Si < 21 min = 1. Si [21, 26) min = 0.5. Si [26, 31) min = 0.25. Si >= 31 min = 0.
calcular_puntaje_gradual <- function(tiempo_min) {
  case_when(
    tiempo_min < 21 ~ 1.00,        # Menos de 21 minutos
    tiempo_min < 26 ~ 0.50,        # Entre 21 y menos de 26 minutos
    tiempo_min < 31 ~ 0.25,       # Entre 26 y menos de 31 minutos
    TRUE ~ 0.00                   # 31 minutos o más
  )
}
origenes <- origenes %>%
  mutate(
...
manzanas_final <- manzanas %>%
  ...
  # Actualizar nombres de columnas para ADMINISTRATIVOS
  rename(
    puntaje_dist_administrativos_PcD = puntaje_PcD,
    puntaje_dist_administrativos_PsD = puntaje_PsD
  )
...
# =======================================
## FUNCIÓN ACTUALIZADA con las categorías de tiempo para ADMINISTRATIVOS
crear_tabla_resumen <- function(datos_sf, perfil) {
  
  col_puntaje <- paste0("puntaje_dist_administrativos_", perfil) # Usar el nuevo nombre de columna
  # Velocidad en metros por minuto (m/min)
  velocidad_mpm <- if (perfil == "PcD") (2.5 * 1000) / 60 else (5 * 1000) / 60
  
  # Categorías de tiempo ACTUALIZADAS para Equipamientos ADMINISTRATIVOS
  categorias_tiempo <- c(
    "1 - Menos de 21 min", 
    "0,5 - Entre 21 y 26 min", 
    "0,25 - Entre 26 y 31 min", 
    "0 - Mas de 31 min"
  )
  umbrales_min <- c(21, 26, 31, NA) # Límite superior para la distancia/tiempo
  ...
  resumen <- tibble(
    ...
    `Distancia Aprox (m)` = c(
      round(21 * velocidad_mpm), # Max 21 min
      round(26 * velocidad_mpm), # Max 26 min
      round(31 * velocidad_mpm), # Max 31 min
      NA
    )
  ) %>%
...
write.csv(tabla_combinada, "C:/Users/pcusu/Desktop/puntaje_distancia_ADMINISTRATIVOS_tabla.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
crear_mapa_accesibilidad <- function(datos_sf, perfil, tabla_resumen) {
  ...
  col_puntaje <- paste0("puntaje_dist_administrativos_", perfil) # Nuevo nombre de columna
  # Título actualizado para Equipamientos Administrativos
  titulo_mapa <- paste("Acceso a Equipamientos Administrativos - Perfil", perfil)
...
st_write(manzanas_final, "C:/Users/pcusu/Desktop/puntaje_distancia_administrativos_equipamientos.geojson", delete_layer = TRUE)
