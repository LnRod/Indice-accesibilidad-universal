...
# *** CÁLCULO DE PUNTAJE DE DESTINOS (Lógica de Facilidades de EDUCACIÓN) ***
destinos_puntaje <- destinos %>%
  mutate(
    # Implementación de la lógica de puntaje basada en la Modalidad de EDUCACIÓN.
    puntaje_base = case_when(
      Modalidad == "Secundario" ~ 0.50,
      Modalidad == "Primario" ~ 0.50,
      Modalidad == "Jardín de Infantes" ~ 0.50,
      Modalidad == "Especial" ~ 0.75,
      Modalidad == "Inicial" ~ 0.50,
      Modalidad == "Escuelas privadas PcD" ~ 0.25,
...
# Encontrar los 5 vecinos más cercanos dentro del umbral de 12 minutos
...
manzanas_final <- manzanas %>%
  ...
  rename(
    # Nombres actualizados para EDUCACIÓN
    puntaje_fac_educacion_PcD = puntaje_acumulado.x,
    puntaje_fac_educacion_PsD = puntaje_acumulado.y
  ) %>%
  mutate(
    # Reemplazar NA (fuera de rango accesible/8 min) con 0.00
    puntaje_fac_educacion_PcD = replace_na(puntaje_fac_educacion_PcD, 0.00),
    puntaje_fac_educacion_PsD = replace_na(puntaje_fac_educacion_PsD, 0.00)
  ) %>%
  # Forzar dos decimales en los puntajes finales
  mutate(
    puntaje_fac_educacion_PcD = round(puntaje_fac_educacion_PcD, 2),
    puntaje_fac_educacion_PsD = round(puntaje_fac_educacion_PsD, 2)
  ) %>%
  select(ID_mza, geometry, puntaje_fac_educacion_PcD, puntaje_fac_educacion_PsD)
...
tabla_pcd <- crear_tabla_rangos_promedio(manzanas_final, "puntaje_fac_educacion_PcD", "PcD")
tabla_psd <- crear_tabla_rangos_promedio(manzanas_final, "puntaje_fac_educacion_PsD", "PsD")
...
st_write(manzanas_final, "C:/Users/pcusu/Desktop/puntaje_facilidades_EDUCACION_8min_promedio.geojson", delete_dsn = TRUE)

# 8.2 Exportar tabla de rangos CSV
write.csv(tabla_combinada, "C:/Users/pcusu/Desktop/resumen_rangos_facilidades_EDUCACION_8min_promedio.csv", row.names = FALSE, fileEncoding = "UTF-8")
